...
- **违规（退出码非 0，必修）**：① 锚点存在（逐字片段确在所指源文件里）② 死链（`[[]]` 解析到真实页/别名）③ 时间线顺序（日期非递减）。
...
**蕴含校验（Tier-B）**：`lint/extract_claims.py` 把每条 `(断言, 引文, 来源)` 拆出，交一个**换实例的 LLM 判官**三分判"引文是否支持断言"（支持/不支持/信息不足），抓"引文真但断言被脑补/拔高/歪曲"。是把可疑项捞给人复核的**筛子**，不自动改。协议见 `lint/entailment_check.md`。数字勾稽、法律定性见设计文档。
