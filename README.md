...
**蕴含校验（Tier-B）**：`lint/extract_claims.py` 把每条 `(断言, 引文, 来源, 源文上下文窗口)` 拆出，交一个**换实例的 LLM 判官**三分判"引文是否支持断言"（支持/不支持/信息不足），抓"引文真但断言被脑补/拔高/歪曲"。是把可疑项捞给人复核的**筛子**，不自动改。协议见 `lint/entailment_check.md`。法律定性正确性归人。
