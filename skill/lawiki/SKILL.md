...
## 校验（lint，EXTRACTED 硬底线的机器闸门）

`lint/lint_wiki.py` 确定性地校验：wiki 每个 `〔来源:…「片段」〕` 的逐字片段是否真的（去格式噪声后）逐字、按序出现在所指源文件里。

- 用法：`python lint/lint_wiki.py <案件根目录>`（根目录下有 `wiki/` 与 `_md/`）。退出码非 0 即有违规。
- 它**只消除格式噪声**（空白/换行/全半角标点/千分位逗号/markdown/表格/HTML 标签），**数字与文字精确**——所以"换行差异"不误报，"数字写错/张冠李戴"必被抓。
- 每次 ingest 后必跑，修到 0 违规再交付。**这是把 EXTRACTED 从自觉变成可机检的那一步。**
