...
8. **跑 lint 校验**：`python lint/lint_wiki.py <案件根目录>`，把所有 flagged 锚点修到 **0 违规**——见下「校验」。
...
`lint/lint_wiki.py` 对案件目录做四类**确定性**检查。用法：`python lint/lint_wiki.py <案件根目录>`（根目录下有 `wiki/` 与 `_md/`）。退出码非 0 即有**违规**。
...
**警告（soft，不影响退出码，交人判断）：**
4. **覆盖率**：`_md/` 下哪些源文件从未被任何锚点引用——可能是漏 ingest 的实质文件，也可能是有意跳过的草稿/红线版。
...
## 蕴含校验（claim↔引文，Tier-B）

锚点 lint 只证明"引文存在"；蕴含校验进一步证明"**断言真能由引文推出**"，抓"引文真、但断言被脑补/拔高/歪曲"（认购→已实缴、拟→已、部分→全体、指控→认定）。流程：`extract_claims.py` 拆 `(断言,引文,来源)` → 交**换实例 LLM 判官**三分判（支持/不支持/信息不足）→ 不支持/信息不足者人工复核。**只喂引文+源文上下文窗口、不喂全文、换实例、是筛子不自动改。** 详见 `lint/entailment_check.md`。

**写作铁规则（由此而来）**：**一条断言 ← 一条能完整支持它的引文**。别把多个事实（日期+轮次+金额+注册资本…）塞在一条锚点下；一句里有几个独立事实就挂几条各自够用的锚点。

> 仍未机检：数字**勾稽闭合**（加总=合计）需页面写显式 `> [!check]` 断言（Tier-B 待做）；法律定性正确性归人。
